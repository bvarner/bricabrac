...
module starKnob(points, handleOuterDiameter, handleInnerDiameter, pointRadius, handleThickness) {
  ...
  difference() {
    ...
    if (boltPitch > 0 && boltHeadHeight > 0) {
          translate([0,0,-baseHeight-1]) metric_thread(boltDiameter, boltPitch, boltHeadHeight+1+err);
    } else {
        if (boltHeadHeight > 0)
